// Import required LangChain modules and web loader
...
const question = "What is @langchain/community?";

/**
 * Main pipeline: loads web page, splits, stores in memory, retrieves, and answers.
 */
async function main(): Promise<void> {
    // Load the web page using Cheerio (HTML parser)
    const loader = new CheerioWebBaseLoader("https://js.langchain.com/docs/introduction/");
    const docs = await loader.load(); // docs: Document[]

    // Split the web page into smaller chunks for embedding
    const splitter = new RecursiveCharacterTextSplitter({
        chunkSize: 200, // Max chunk size
